// c++ -o compute HPGecalibration.cpp ../../lib/analyzer.cc `root-config --cflags --glibs`
...
void computeHisto(string value, int bin, double limInf, double limSup, double parFit0, double parFit1, double parFit2, double parFit3) {
    Analyzer HPGe;
    string fileName = "data/HPGe-Calibration/" + value + ".dat";
    ...
    if (HPGe.loadData(fileName, data, min, max) == false) {
    ...
    TString titleGraph = "Sorgente " + value;
    TString namePDF = "graphs/spectreCalibration/graph_" + value + ".pdf";
...
int main() {
    computeHisto("22Na", 8192, 0, 9000, 0, 0, 0, 0);
    computeHisto("60Co", 8192, 0, 9000, 0, 0, 0, 0);
    computeHisto("228Th", 8192, 0, 9000, 0, 0, 0, 0);
}
